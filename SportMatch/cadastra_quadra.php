...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto_quadra']) && $_FILES['foto_quadra']['error'] === 0) {
        ...
        $destino = __DIR__ . "/assets/img/quadras/" . $novoNome;

        if (!is_dir(__DIR__ . "/assets/img/quadras/")) {
            mkdir(__DIR__ . "/assets/img/quadras/", 0777, true);
        }

        if (move_uploaded_file($_FILES['foto_quadra']['tmp_name'], $destino)) {
            $nomeImagem = $novoNome;
        } else {
...
<body class="body-form"> <!-- Classe Mestra para Fundo Escuro -->

    <?php include BASE_PATH . '/pages/includes/navbar.php'; ?>

    <main class="main-content">
        <div class="form-box"> <!-- Classe padronizada -->
            <h2>Cadastrar Nova Quadra</h2>

            <?php if($mensagem): ?>
                <div class="msg-success"><?php echo $mensagem; ?> <br> <a href="quadras.php" style="text-decoration: underline;">Ver na lista</a></div>
            <?php endif; ?>
            <?php if($erro): ?>
